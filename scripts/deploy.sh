...
echo "================================"
echo "VumGames Django Deployment"
echo "================================"
...
if [ ! -f "$PROJECT_DIR/.env" ]; then
    ...
    echo ""
    echo "IMPORTANT: Edit /var/www/vumgames/.env and configure:"
...
sudo cp $PROJECT_DIR/nginx.conf /etc/nginx/sites-available/vumgames
sudo ln -sf /etc/nginx/sites-available/vumgames /etc/nginx/sites-enabled/
...
if [ "$setup_ssl" = "y" ]; then
    ...
    sudo certbot --nginx -d vumgames.com -d www.vumgames.com --non-interactive --agree-tos --email $email --redirect
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "sudo certbot --nginx -d vumgames.com -d www.vumgames.com"
    fi
else
...
if [ "$setup_ssl" = "y" ]; then
    ...
    echo "  Admin panel: https://vumgames.com/admin/"
else
    echo "  http://vumgames.com (HTTP only - setup SSL for HTTPS)"
    echo "  Admin panel: http://vumgames.com/admin/"
fi
echo ""
